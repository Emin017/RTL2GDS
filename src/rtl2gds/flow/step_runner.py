...
from .. import step
from ..global_configs import StepName, RTL2GDS_FLOW_STEP
...
class StepRunner:
    ...
    def run_pr_step(self, step_name: str):
        ...
        step_file_prefix = f"{self.chip.path_setting.result_dir}/{self.chip.design_top}_{step_name}"
    ...
    def run_dump_layout_gds(self, step_name: str, take_snapshot: bool = False):
        ...
        step.dump_layout_gds.run(
            ...
            result_dir=self.chip.path_setting.result_dir,
            snapshot_file=f"{self.chip.path_setting.result_dir}/{self.chip.design_top}_{step_name}.png" if take_snapshot else None,
        )
