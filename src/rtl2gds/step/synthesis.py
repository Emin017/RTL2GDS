...
def run(
    ...
    yosys_report_dir = f"{design_path.result_dir}/yosys"
    os.makedirs(yosys_report_dir, exist_ok=True)

    rtl_file = design_path.rtl_file
    if isinstance(rtl_file, list) and isinstance(rtl_file[0], str):
        rtl_file = " \n ".join(rtl_file)
    ...
    step_env = {
        ...
        "YOSYS_REPORT_DIR": yosys_report_dir,
        "CLK_PORT_NAME": design_constrain.clk_port_name,
        "CLK_FREQ_MHZ": design_constrain.clk_freq_mhz,
    }

    if design_constrain.die_area and design_constrain.core_area:
        step_env.update(
            {
        ...
        assert (
            design_constrain.core_util is not None
    ...
    step_env.update(global_configs.ENV_TOOLS_PATH)
    ...
    assert os.path.exists(design_path.netlist_file), "Netlist file not found"
    ...
    if not (design_constrain.die_area and design_constrain.core_area):
        core_length = math.sqrt(cell_area / design_constrain.core_util)
        io_margin = 10
        design_constrain.die_area = (
            f"0 0 {core_length+io_margin*2} {core_length+io_margin*2}"
        )
        design_constrain.core_area = (
            f"{io_margin} {io_margin} {core_length+io_margin} {core_length+io_margin}"
        )

    elif not design_constrain.core_util:
        core = design_constrain.core_area.split(" ")
        core_len_x = float(core[2]) - float(core[0])
        core_len_y = float(core[3]) - float(core[1])
        core_area = core_len_x * core_len_y
        design_constrain.core_util = cell_area / core_area
        assert 0 < design_constrain.core_util < 1, "Core utilization out of range"

    else:
        assert 0

    return design_constrain
    # {
    #     "DIE_AREA": design_constrain.die_area,
    #     "CORE_AREA": design_constrain.core_area,
    #     "CORE_UTIL": design_constrain.core_util,
    # }
