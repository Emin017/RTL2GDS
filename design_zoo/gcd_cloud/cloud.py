    ...
    try:
        ...
        if len(command_parts) < 2:
            ...
            return ResponseModel(
                code=400,
                message="Bad Request: Command must be in format 'run step_name'",
                data=ResponseData(code="INVALID_COMMAND_FORMAT"),
            )
        ...
        if step_name not in RTL2GDS_FLOW_STEPS:
            ...
            return ResponseModel(
                code=404,
                message=f"Not Found: Step '{step_name}' is not a valid flow step",
        ...
        if not mount_point.is_dir():
            logging.error(
                f"PVC mount point {mount_point} not found or is not a directory inside the pod."
            )
            return ResponseModel(
                code=503,
        ...
        if "Failed" in response_message:
            return ResponseModel(
                code=500,
                message="Internal Server Error: Failed to create backend job",
                data=ResponseData(code="JOB_CREATION_FAILED"),
            )
        else:
            return ResponseModel(
                code=0,
                message="Accepted: Job submitted successfully",
                data=ResponseData(code="JOB_SUBMITTED"),
            )
